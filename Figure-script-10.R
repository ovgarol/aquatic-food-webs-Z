...
for(idx in what.to.plot){
  ...
  for(i in 1:length(db.mm$tag))for(j in 1:length(esds)){
    if(!represented[j])if((opss[j]<exp(-1.65+log(esds[j])-0.011*log(esds[j])**2+1.*error.limit)) & (opss[j]>exp(-1.65+log(esds[j])-0.011*log(esds[j])**2-1.*error.limit))){
      represented[j] = 1
      #touched[i] = 1
    }  
  }
  ...
  if(T){
    ...
    if(T){
      x = seq(log(min(esds))*0.9,log(max(esds))*1.1,by=0.2)#log(10^(-60:100 * 0.1)*1e6 )
      y = exp(-1.65+x-0.011*x**2)
      x = exp(x)
      #lines(x,y)
      #polygon(c(x,rev(x)),c(1/(3*error.limit)*y,rev(3*error.limit*y)),border='red',col=alpha('gray',0.5),lty=0) 
    }
